...
public void setUp(){
	...
	public void testReturnBookRecords(){
		List<BookRecord> records=Arrays.asList(
				new BookRecord(ISBN2, null, READER_ID2));
		int ind=0;
	...
	public void testDelayedRecords(){
		assertEquals(false,
				library.getDelayedBookRecords(CURRENT_DATE_NONDELAY)
				.iterator().hasNext());
	...
	public void testDelayedReaders(){
		assertEquals(false,library.getReadersDelayedBooks(CURRENT_DATE_DELAY, BY_DAYS_DELAY+1).
				iterator().hasNext());
