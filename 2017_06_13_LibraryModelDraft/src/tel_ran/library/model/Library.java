...
public Library(int pickPeriod){
	...
	abstract public boolean removeBook(long isbn); //with all records. only if the book exists and not in use.
	...
	abstract public Iterable<BookRecord> getDelayedBookRecords(LocalDate currentDate);
	//getting info about the readers who have been delaying books for
	//more than the given byDays value
	abstract public Iterable<Reader> getReadersDelayedBooks(LocalDate currentDate,int byDays);
	abstract public boolean addReader(Reader reader);
	abstract public Iterable<Reader> getReaders(int isbn);
	...
	abstract public boolean removeBookExemplar(long isbn);
