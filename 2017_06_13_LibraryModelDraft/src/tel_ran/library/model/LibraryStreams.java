...
public class LibraryStreams extends Library {
	public LibraryStreams(int pickPeriod) {
		super(pickPeriod);
	}
	...
	public Iterable<BookRecord> getNonReturnedBookRecords() {
		return getRecordsStream().collect(Collectors.toList());
	}

	public Stream<BookRecord> getRecordStream() {
		return nonreturnedRecords.values().stream().flatMap(rl -> rl.stream());
	}
	...
	public boolean removeBook(long isbn) {
		...
		if (nonReturnedBooksSet.contains(isbn))
			return false;
	...
	public Iterable<Book> getNonReturnedBooks() {
		return getRecordsStream().map(r -> books.get(r.getIsbn())).distinct().collect(Collectors.toList());
	}
	...
	@Override
	public Iterable<BookRecord> getDelayedBookRecords(LocalDate pickDate) {
		return getDelayedBookRecordsStream(pickDate).collect(Collectors.toList());
	}

	public Stream<BookRecord> getDelayedBookRecordsStream(LocalDate pickDate) {
		return nonreturnedRecords.headMap(pickDate).values().stream().flatMap(rl -> rl.stream());
	}
	...
	public void displayMostLeastPopularBooksRecords(Stream<BookRecord> records, boolean isMost) {
		Map<Long, Long> mapOccurences = records
				.collect(Collectors.groupingBy(BookRecord::getIsbn, Collectors.counting()));
		long max = mapOccurences.entrySet().stream().max(
				(x, y) -> isMost ? Long.compare(x.getValue(), y.getValue()) : Long.compare(y.getValue(), x.getValue()))
				.get().getValue();
		mapOccurences.entrySet().stream().filter(e->e.getValue()==max).forEach(e->System.out.println(this.getBookItem(e.getValue())));
	}
}
