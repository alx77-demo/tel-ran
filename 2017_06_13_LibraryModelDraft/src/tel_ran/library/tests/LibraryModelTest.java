...
public class LibraryModelTest {
	...
	private static final LocalDate CURRENT_DATE_DELAY = LocalDate.parse("2017-05-30");
	private static final LocalDate CURRENT_DATE_NONDELAY = LocalDate.parse("2017-05-10");
	private static final BookRecord RECORD_ISBN1 = new BookRecord(ISBN1, null, READER_ID1);
	private static final BookRecord RECORD_ISBN2 = new BookRecord(ISBN2, null, READER_ID2);
	...
	public void testAddBookItem() {
		...
		assertEquals(true, library.addBookItem(new Book(128, "hhh", "author", 2)));
	}
	...
	public void testAddReader() {
		...
		assertEquals(true, library.addReader(new Reader(128, "name", "phone", LocalDate.now())));
	}
	...
	public void testReturnBookRecords() {
		List<BookRecord> records = Arrays.asList(new BookRecord(ISBN2, null, READER_ID2));
		int ind = 0;
	...
	public void testDelayedRecords() {
		assertEquals(false, library.getDelayedBookRecords(CURRENT_DATE_NONDELAY).iterator().hasNext());
		for (BookRecord record : library.getDelayedBookRecords(CURRENT_DATE_DELAY))
			assertEquals(RECORD_ISBN2, record);
	}

	@Test
	public void testDelayedReaders() {
		assertEquals(false, library.getReadersDelayedBooks(CURRENT_DATE_DELAY, BY_DAYS_DELAY + 1).iterator().hasNext());
		for (Reader reader : library.getReadersDelayedBooks(CURRENT_DATE_DELAY, BY_DAYS_DELAY - 1))
